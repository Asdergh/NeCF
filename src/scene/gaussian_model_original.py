...
class GaussianModel:
    ...
    def setup_functions(self):
        def build_covariance_from_scaling_rotation(scaling, scaling_modifier, rotation):
            ...
            print(actual_covariance.size(), "ACtual covariance")
            # symm = strip_symmetric(actual_covariance)
            # print(symm.size(), "after ssym")
    ...
    def load_nifti(
        ...
        scales = 0.5 * torch.ones(N, 3)
        quats = torch.ones(N, 4)
        
        self._xyz = nn.Parameter(points_xyz.requires_grad_(True))
